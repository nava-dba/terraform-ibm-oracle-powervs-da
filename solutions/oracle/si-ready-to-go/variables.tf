...
variable "powervs_aix_image_name" {
  description = "Name of the AIX boot image for Oracle Database instance. Must exist in the PowerVS workspace."
  type        = string
}

variable "powervs_aix_instance" {
  description = "Configuration for PowerVS AIX instance hosting Oracle Database."
  type = object({
    memory_gb     = number
    cores         = number
    core_type     = string
  ...
  validation {
    condition     = var.powervs_aix_instance.memory_gb >= 16
    error_message = "AIX instance memory must be at least 16GB for Oracle Database."
  }

  validation {
    condition     = var.powervs_aix_instance.cores >= 0.25
    error_message = "AIX instance must have at least 0.25 cores."
  }
...
variable "oracle_install_type" {
  description = "Oracle installation type: 'ASM' (Automatic Storage Management) or 'JFS2' (Journal File System)."
  type        = string
  default     = "ASM"

  validation {
    condition     = contains(["ASM", "JFS2"], var.oracle_install_type)
    error_message = "oracle_install_type must be either 'ASM' or 'JFS2'."
  }
}
...
variable "powervs_data_volume" {
  description = "Storage configuration for Oracle DATA (ASM diskgroup or JFS2 filesystem)."
  type = object({
    size  = string
    count = string
    tier  = string
...
variable "powervs_redo_volume" {
  description = "Storage configuration for Oracle REDO logs (ASM diskgroup or JFS2 filesystem)."
  type = object({
    size  = string
    count = string
    tier  = string
...
variable "oracle_sid" {
  description = "Oracle System Identifier (SID) for the database instance. Must be alphanumeric, max 8 characters."
  type        = string
...
variable "ibmcloud_cos_configuration" {
  description = "IBM Cloud Object Storage configuration for Oracle binaries."
  type = object({
    cos_region                    = string
