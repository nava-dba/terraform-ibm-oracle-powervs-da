...
| External Access IP (CIDR) | external_access_ip | This is the IP address of the machine from where you want to ssh into the deployed resources. It could be the IP of the workstation or CIDR of the IP address. | Example IP: 203.1.113.5/16, Example CIDR 203.1.0.0/16. Using 0.0.0.0/0 is insecure and allows login from any machine |
...
| VPC Subnet CIDRs (Optional) | vpc_subnet_cidrs | This needs to be configured when Client-to-Site VPN is required. CIDR values for the VPC subnets to be created. Must be new IP pools to avoid collision with existing networks. It is customer responsibility that none of the defined networks collide, including the PowerVS subnets and VPN client pool. Refer to [Power Virtual Server Landing Zone](https://cloud.ibm.com/docs/powervs-vpc?topic=powervs-vpc-landing-zone-preset). | |
